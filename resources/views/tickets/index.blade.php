            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between md:items-center mb-4">
                        ...
                        <a href="{{ route('tickets.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 mb-2 md:mb-0">
                            + Buat Tiket Baru
                        </a>
                    ...
                    <div class="flex items-center space-x-2 border-t border-b py-2 mb-4">
                        ...
                        <a href="{{ route('tickets.index') }}" class="px-3 py-1 text-sm rounded-full {{ !request('filter') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">Semua Tiket</a>
                        <a href="{{ route('tickets.index', ['filter' => 'mine']) }}" class="px-3 py-1 text-sm rounded-full {{ request('filter') == 'mine' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">Tiket Saya</a>
                        <a href="{{ route('tickets.index', ['filter' => 'unassigned']) }}" class="px-3 py-1 text-sm rounded-full {{ request('filter') == 'unassigned' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">Belum Ditugaskan</a>
                    </div>
                    @endif
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($tickets as $ticket)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ Str::limit($ticket->title, 25) }}</td>
                                        @if(Auth::user()->is_admin)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $ticket->assignedTo->name ?? '-' }}</td>
